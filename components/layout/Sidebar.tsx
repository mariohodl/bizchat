...
export function Sidebar() {
  ...
  return (
    <>
      ...
      <aside className={`fixed left-0 top-0 h-full z-30 bg-white border-r border-slate-200 flex flex-col transition-all duration-300 ${sidebarOpen ? "w-64" : "w-20"}`}>
        <div className="flex items-center justify-between px-5 h-20 border-b border-slate-100 flex-shrink-0">
          {sidebarOpen && (
            <div className="flex items-center gap-3">
              ...
              <span className="font-black text-lg tracking-tight text-slate-900">BizChat <span className="text-emerald-600">MX</span></span>
            </div>
          )}
          {!sidebarOpen && (
            <div className="w-10 h-10 rounded-2xl bg-emerald-600 flex items-center justify-center mx-auto shadow-lg shadow-emerald-600/20">
              <Zap className="w-5 h-5 text-white" strokeWidth={2.5} />
            </div>
          )}
        </div>
