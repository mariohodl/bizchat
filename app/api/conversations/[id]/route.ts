...
export async function GET(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const conv = await Conversation.findOne({ _id: id, businessId: (session.user as any).businessId })
      .populate("customerId", "name phone email tags").lean()
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const conv = await Conversation.findOne({ _id: id, businessId: bId }).populate("customerId")
    if (!conv) return NextResponse.json({ error: "No encontrado" }, { status: 404 })

    const message = { content, type: templateId ? "template" : "text", direction: "outbound", status: "sent", sentAt: new Date(), sentBy: (session.user as any).id, isAutomated: false, templateId }
